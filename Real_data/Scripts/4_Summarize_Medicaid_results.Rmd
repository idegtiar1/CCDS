...
table_linear_STSM = table_linear %>% filter(Estimator %in% c("STSM rand OR", "STSM obs OR", # OR = outcome regression
                                                             "STSM rand AIPW", "STSM rand TMLE", 
                                         "STSM obs AIPW", "STSM obs TMLE")) %>% 
  mutate(Estimator2 = ifelse(Estimator %in% c("STSM rand AIPW", "STSM obs AIPW"), "AIPW",
                       ifelse(Estimator %in% c("STSM rand TMLE", "STSM obs TMLE"), "TMLE",
                              ifelse(Estimator %in% c("STSM rand OR", "STSM obs OR"), "OR",
                              "Other"))),
         Data = ifelse(Estimator %in% c("STSM rand OR", "STSM rand AIPW", "STSM rand TMLE"), "rand",
    ...
    mutate(Estimator2 = factor(Estimator2, levels = c("OR", "AIPW", "TMLE")),
           Data = factor(Data, levels = c("rand", "obs")))

# Re-order plans based on rand study averages  
levels_A_sorted = levels(fct_reorder(table_linear_STSM[table_linear_STSM$Data=="rand","Intervention"], 
                                     table_linear_STSM[table_linear_STSM$Data=="rand","Estimate"], mean)) 
...
plot_results(table_linear %>% filter(Estimator %in% c("STSM rand AIPW", "STSM obs AIPW",   
                                                      "Rand OR", "Obs/rand OR",
...
# Add color grouping for estimators
table_ensemble_STSM = table_ensemble %>% filter(Estimator %in% c("STSM rand OR", "STSM obs OR",
                                                             "STSM rand AIPW", "STSM rand TMLE", 
                                         "STSM obs AIPW", "STSM obs TMLE")) %>% 
  mutate(Estimator2 = ifelse(Estimator %in% c("STSM rand AIPW", "STSM obs AIPW"), "AIPW",
                       ifelse(Estimator %in% c("STSM rand TMLE", "STSM obs TMLE"), "TMLE",
...
mean_alg_weights_sim %>%
  ...
  set_colnames(c("S", "Roverlap_rand","Roverlap_obs",
                 ...
                 "rand","obs","rand_overlap","obs_overlap")) %>%
  round(2) %>% kable("html") %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover"))
...
# Plot results for subset of CCDS estimators
plot_results(table_ensemble %>% filter(Estimator %in% c("STSM rand AIPW", "STSM obs AIPW",  
                                                      "Rand OR", "Obs/rand OR",
                                                      "CCDS-AIPW", "2-stage CCDS")), levels_A_sorted) + 
  facet_wrap(~ factor(Intervention, levels_A_sorted), nrow = 2)   
```

## Differences CCDS-AIPW vs. rand estimates
...
```{r rand_aipw_diff}
# Difference between CCDS-AIPW vs. rand estimates
table_diff = table_ensemble  %>%
    group_by(Intervention) %>%
    arrange(Estimator ) %>%
    summarize(diff = Estimate[Estimator == "CCDS-AIPW"] - Estimate[Estimator == "Rand OR"],
           p_diff = diff/Estimate[Estimator == "Rand OR"]*100,
           diff_dollar = exp(Estimate[Estimator == "CCDS-AIPW"]) - exp(Estimate[Estimator == "Rand OR"])) %>% 
...
As also seen in the simulation, impact estimates are minimally impacted: CCDS-AIPW estimates shift slightly closer to the rand estimates (as expected, since almost all the data being in the overlap region brings estimates closer to the rand estimator); 2-stage CCDS estimates are not notably affected; CCDS-IPW's estimates were most impacted (as also seen in the simulation) -- its point estimates shifted and CI shrunk.
...
# Add color grouping for estimators
table_linear_overlap2_STSM = table_linear_overlap2 %>% filter(Estimator %in% c("STSM rand OR", "STSM obs OR",
                                                             "STSM rand AIPW", "STSM rand TMLE", 
                                         "STSM obs AIPW", "STSM obs TMLE")) %>% 
  mutate(Estimator2 = ifelse(Estimator %in% c("STSM rand AIPW", "STSM obs AIPW"), "AIPW",
                       ifelse(Estimator %in% c("STSM rand TMLE", "STSM obs TMLE"), "TMLE",
...
# Plot results for subset of CCDS estimators
plot_results(table_linear_overlap2 %>% filter(Estimator %in% c("STSM rand AIPW", "STSM obs AIPW",  
                                                      "Rand OR", "Obs/rand OR",
                                                      "CCDS-AIPW", "2-stage CCDS")), levels_A_sorted) + 
  facet_wrap(~ factor(Intervention, levels_A_sorted), nrow = 2)   
```
