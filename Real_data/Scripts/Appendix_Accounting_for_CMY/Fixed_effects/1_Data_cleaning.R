...
# Author: ""
# Date: "7/1/21"
# Description: Medicaid analysis using fixed effects; 
#              same as main analysis but also keeps country-month-year variable
#########################################################################################################
...
X_var = c("female", "race_num", "recip_age", "aid_group", "ssi", #"recip_county_fips"
...
target_sample_6mo_wide = target_sample_6mo_subset %>% pivot_wider(names_from = post_assign_month, 
                                                                   values_from = c(pay0_tot, plan, plan_assign, recip_age, 
                                                                                   aid_group, zip, ssi, countyyearmonthfe), 
                                                                   values_fill=list(0,"0",NA,NA,NA,NA, NA)) %>%
  select(-c(plan_1,plan_2,plan_3,plan_4,plan_5,
            ...
            countyyearmonthfe_1,countyyearmonthfe_2,countyyearmonthfe_3,countyyearmonthfe_4,countyyearmonthfe_5)) %>%
  rename(plan_baseline = plan_0,
         ...
         zip = zip_0,
         ssi = ssi_0,
         countyyearmonthfe = countyyearmonthfe_0)
...
target_sample_6mo_wide$pay0tot_0to5 = apply(subset(target_sample_6mo_wide, select=pay0_tot_0:pay0_tot_5),1,sum) #select=`0`:`5`),1,sum)#
...
vars_to_merge = c("recip_id","plan_baseline","plan_assign_baseline","age","aid_group","zip","ssi", "countyyearmonthfe")
benes_to_merge = target_sample_6mo[match(unique(benes_missing$recip_id), target_sample_6mo$recip_id),] %>% 
  select(c("recip_id","plan","plan_assign","recip_age","aid_group","zip","ssi", "countyyearmonthfe"))
...
X_var2 = c("female", "age_clean120", "aid_group_clean120", "ssi", #"recip_county_fips"
          ...
          "percent_poverty", "countyyearmonthfe")# sex, race, age, aid group, SSI eligibility, neighborhood, baseline spending decile, country month year
...
qsave(target_sample_6mo_wide, "/data/NYS_Medicaid/Name/target_sample_6mo_wide_cmy.qs") # all AC/AA subjects -- will use to compare characteristics
qsave(target_sample_relevant, "/data/NYS_Medicaid/Name/target_sample_relevant_cmy.qs") # Restricted to those with 6 mo of observations (can measure outcome) -- will use for analysis
qsave(target_sample_relevant, "/data/NYS_Medicaid/Name/target_sample_relevant1500_cmy.qs")
