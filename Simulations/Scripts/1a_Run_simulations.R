...
### Author: 
### Description: Run paper 2 simulations across all data generating mechanisms; fast ensemble library used
####################################################################################################
...
## ----run_sim_complex_misspecified_t_o-------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_t_o_linear = data.frame(t(parSapply(cl,1:m,
...
# Save results
saveRDS(sim_results_t_o_linear,
        "/Results/Simulation_results/sim_results_t_o_linear.rds")

# `----- run_sim_complex_misspecified
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_linear = data.frame(t(parSapply(cl,1:m,
               function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                  target_pop = sim_data_complex_noX1cube, # target population from which to draw
                                  observed_U = F, # whether U is observed
                                  overlap_function = F)}))) # true overlap region or way to determine overlap (F=default, overlap1, overlap2, overlap3)



# Save results
saveRDS(sim_results_linear,
        "/Results/Simulation_results/sim_results_linear.rds")

# ## ----run_sim_complex_sq_t_o-----------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_t_o_sq = data.frame(t(parSapply(cl,1:m,
               function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                  target_pop=sim_data_complex_X1sq, # target population from which to draw
                                  observed_U=F, # whether U is observed
                                  overlap_function="ifelse(X_target[,1]>qnorm(0.9),0,
                                  ifelse(X_target[,1]<qnorm(0.5),0,1))")}))) # function for overlap region



# Save results
saveRDS(sim_results_t_o_sq,
        "/Results/Simulation_results/sim_results_t_o_sq.rds")


## ----run_sim_cs_t_o-------------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_cs_t_o = data.frame(t(parSapply(cl,1:m,
               function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                  target_pop=sim_data_complex, # target population from which to draw
                                  observed_U=F,
                                  complex_fit_models="correctly_specified",
                                  overlap_function="ifelse(X_target[,1]>qnorm(0.9),0,
                                  ifelse(X_target[,1]<qnorm(0.5),0,1))")}))) # whether U is observed #
...
# Save results
saveRDS(sim_results_cs_t_o,
        "/Results/Simulation_results/sim_results_cs_t_o.rds")


## ----run_sim_cs-----------------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_cs = data.frame(t(parSapply(cl,1:m,
               function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                  target_pop=sim_data_complex, # target population from which to draw
                                  observed_U=F, # whether U is observed
                                  complex_fit_models="correctly_specified",
                                  S_ridge=T, # because otherwise no overlap region
                                  overlap_function=F)})))  #



# Save results
saveRDS(sim_results_cs,
        "/Results/Simulation_results/sim_results_cs.rds")
...
sim_results_ensemble = parSapply(cl,1:m,
               function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size #
                                  ...
                                  overlap_function=F)}) # whether U is observed #
...
# ## ----run_sim_ksvm_t_o-----------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_ksvm_t_o = parSapply(cl,1:m,
               function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size #
                                  target_pop=sim_data_complex_noX1cube, # target population from which to draw
                                  observed_U=F,
                                  complex_fit_models="ksvm", # whether to use ksvm or random forest (ranger) to fit models for Y, A, and S
                                  overlap_function="ifelse(X_target[,1]>qnorm(0.9),0,
                                  ifelse(X_target[,1]<qnorm(0.5),0,1))")}) # whether U is observed #
...
# Save results
saveRDS(sim_results_ksvm_t_o,
        "/Results/Simulation_results/sim_results_ksvm_t_o.rds")

## ----run_sim_ksvm---------------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_ksvm = parSapply(cl,1:m,
               function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size #
                                  target_pop=sim_data_complex_noX1cube, # target population from which to draw
                                  observed_U=F,
                                  complex_fit_models="ksvm", # whether to use ksvm or random forest (ranger) to fit models for Y, A, and S
...
# Save results
saveRDS(sim_results_ksvm,
        "/Results/Simulation_results/sim_results_ksvm.rds")



## ----run_sim_n200_cs_t_o-------------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_n200_cs_t_o = data.frame(t(parSapply(cl,1:m,
...
# Save results
saveRDS(sim_results_n200_cs_t_o,
        "/Results/Simulation_results/sim_results_n200_cs_t_o.rds")


## ----run_sim_n250_cs_t_o-------------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_n250_cs_t_o = data.frame(t(parSapply(cl,1:m,
                                                 function(i,...){run_simulation_remove_errors(n_target = 250, # target sample size
                                                                                              target_pop=sim_data_complex, # target population from which to draw
                                                                                              observed_U=F,
                                                                                              complex_fit_models="correctly_specified",
                                                                                              overlap_function="ifelse(X_target[,1]>qnorm(0.9),0,
...
# Save results
saveRDS(sim_results_n250_cs_t_o,
        "/Results/Simulation_results/sim_results_n250_cs_t_o.rds")

## ----run_sim_n500_cs_t_o-------------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_n500_cs_t_o = data.frame(t(parSapply(cl,1:m,
                                                 function(i,...){run_simulation_remove_errors(n_target = 500, # target sample size
                                                                                              target_pop=sim_data_complex, # target population from which to draw
                                                                                              observed_U=F,
                                                                                              complex_fit_models="correctly_specified",
                                                                                              overlap_function="ifelse(X_target[,1]>qnorm(0.9),0,
...
# Save results
saveRDS(sim_results_n500_cs_t_o,
        "/Results/Simulation_results/sim_results_n500_cs_t_o.rds")

## ----run_sim_n2000_cs_t_o-------------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_n2000_cs_t_o = data.frame(t(parSapply(cl,1:m,
...
# Save results
saveRDS(sim_results_n2000_cs_t_o,
        "/Results/Simulation_results/sim_results_n2000_cs_t_o.rds")

## ----run_sim_n20000_cs_t_o-------------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_n20000_cs_t_o = data.frame(t(parSapply(cl,1:m,
                                                   function(i,...){run_simulation_remove_errors(n_target = 20000, # target sample size
                                                                                                target_pop=sim_data_complex, # target population from which to draw
                                                                                                observed_U=F,
                                                                                                complex_fit_models="correctly_specified",
                                                                                                overlap_function="ifelse(X_target[,1]>qnorm(0.9),0,
...
# Save results
saveRDS(sim_results_n20000_cs_t_o,
        "/Results/Simulation_results/sim_results_n20000_cs_t_o.rds")

## ----run_sim_n50000_cs_t_o-------------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_n50000_cs_t_o = data.frame(t(parSapply(cl,1:m,
...
# Save results
saveRDS(sim_results_n50000_cs_t_o,
        "/Results/Simulation_results/sim_results_n50000_cs_t_o.rds")
...
# ----run_sim_n200_ksvm_t_o-----------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_n200_ksvm_t_o = parSapply(cl,1:m,
                                      function(i,...){run_simulation_remove_errors(n_target =200, # target sample size
                                                                                   target_pop=sim_data_complex_noX1cube, # target population from which to draw
                                                                                   observed_U=F,
                                                                                   complex_fit_models="ksvm",
                                                                                   overlap_function="ifelse(X_target[,1]>qnorm(0.9),0,
                                  ifelse(X_target[,1]<qnorm(0.5),0,1))")}) # whether U is observed #
...
# Save results
saveRDS(sim_results_n200_ksvm_t_o,
        "/Results/Simulation_results/sim_results_n200_ksvm_t_o.rds")
...
# ----run_sim_n2000_ksvm_t_o-----------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_n2000_ksvm_t_o = parSapply(cl,1:m,
                                       function(i,...){run_simulation_remove_errors(n_target =2000, # target sample size
                                                                                    target_pop=sim_data_complex_noX1cube, # target population from which to draw
                                                                                    observed_U=F,
                                                                                    complex_fit_models="ksvm",
...
# Save results
saveRDS(sim_results_n2000_ksvm_t_o,
        "/Results/Simulation_results/sim_results_n2000_ksvm_t_o.rds")
...
# # ----run_sim_n50000_ksvm_t_o----------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_n50000_ksvm_t_o = parSapply(cl,1:m,
                                        function(i,...){run_simulation_remove_errors(n_target =50000, # target sample size
                                                                                     target_pop=sim_data_complex_noX1cube, # target population from which to draw
                                                                                     observed_U=F,
                                                                                     complex_fit_models="ksvm",
...
# Save results
saveRDS(sim_results_n50000_ksvm_t_o,
        "/Results/Simulation_results/sim_results_n50000_ksvm_t_o.rds")
...
## ----run_sim_complex2_misspecified_t_o-----------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_t_o_linear = data.frame(t(parSapply(cl,1:m,
                                                function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                               ...
                                                                               observed_U = F, # whether U is observed
                                                                               overlap_function = "ifelse(X_target[,1]>qnorm(0.9),0,
                                  ifelse(X_target[,1]<qnorm(0.5),0,1))")}))) # true overlap region or way to determine overlap (F=default, overlap1, overlap2, overlap3)



# Save results
saveRDS(sim_results_t_o_linear,
        "/Results/Simulation_results/sim_results_complex2_t_o_linear.rds")


# ------ run_sim_complex2_misspecified-----------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_linear = data.frame(t(parSapply(cl,1:m,
                                            function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                           target_pop = sim_data_complex2_noX1cube, # target population from which to draw
...
# Save results
saveRDS(sim_results_linear,
        "/Results/Simulation_results/sim_results_complex2_linear.rds")


## ----run_sim_complex2_cs_t_o-----------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_complex2_cs_t_o = data.frame(t(parSapply(cl,1:m,
               function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                  target_pop=sim_data_complex2, # target population from which to draw
                                  observed_U=F,
                                  complex_fit_models="correctly_specified",
                                  overlap_function="ifelse(X_target[,1]>qnorm(0.9),0,
                                  ifelse(X_target[,1]<qnorm(0.5),0,1))")}))) # whether U is observed #
...
# Save results
saveRDS(sim_results_complex2_cs_t_o,
        "/Results/Simulation_results/sim_results_sto_complex2_true.rds")
...
## ----run_sim_complex2_ksvm_t_o--------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_complex2_ksvm_t_o = parSapply(cl,1:m,
               function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size #
                                  target_pop=sim_data_complex2_noX1cube, # target population from which to draw
                                  observed_U=F,
                                  complex_fit_models="ksvm", # whether to use ksvm or random forest (ranger) to fit models for Y, A, and S
...
# Save results
saveRDS(sim_results_complex2_ksvm_t_o,
        "/Results/Simulation_results/sim_results_complex2_ksvm_t_o.rds")
...
## ----run_sim_complex2_ksvm--------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_complex2_ksvm = parSapply(cl,1:m,
               function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size #
                                  target_pop=sim_data_complex2_noX1cube, # target population from which to draw
                                  observed_U=F,
                                  complex_fit_models="ksvm", # whether to use ksvm or random forest (ranger) to fit models for Y, A, and S
...
#
# Save results
saveRDS(sim_results_complex2_ksvm,
        "/Results/Simulation_results/sim_results_complex2_ksvm.rds")
...
## ----run_sim_linear-------------------------------------------------------------------------------------------------------------------------------------------------
# Run simulation - will get warning about rank-deficient fit because U=0 for all randomized individuals
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_linear = data.frame(t(parSapply(cl,1:m,
                                            function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
...
## ----run_sim_linear_t_o---------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_linear_t_o = data.frame(t(parSapply(cl,1:m,
                                                function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                                             target_pop=sim_data_linear, # target population from which to draw
                                                                                             observed_U=F,
                                                                                             complex_fit_models="correctly_specified",
...
# Save results
saveRDS(sim_results_linear_t_o,
        "/Results/Simulation_results/sim_results_linear_t_o.rds")
...
# ----run_sim_t_o_complex--------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_linear_ksvm_t_o = parSapply(cl,1:m,
                                        function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                                     target_pop=sim_data_linear, # target population from which to draw
                                                                                     observed_U=F,
                                                                                     complex_fit_models="ksvm",
...
# Save results
saveRDS(sim_results_linear_ksvm_t_o,
        "/Results/Simulation_results/sim_results_linear_ksvm_t_o.rds")
...
## ----run_sim_knot_cs_t_o--------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_knot_cs_t_o = data.frame(t(parSapply(cl,1:m,
               function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                  target_pop=sim_data_knot_withknot, # target population from which to draw
                                  observed_U=F,
                                  complex_fit_models="correctly_specified",
                                  overlap_function="ifelse(X_target[,1]>qnorm(0.9),0,
                                  ifelse(X_target[,1]<qnorm(0.5),0,1))")}))) # whether U is observed #
...
# Save results
saveRDS(sim_results_knot_cs_t_o,
        "/Results/Simulation_results/sim_results_knot_cs_t_o.rds")
...
## ----run_sim_knot_ksvm_t_o------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_knot_ksvm_t_o = parSapply(cl,1:m,
               function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                  target_pop=sim_data_knot_noX1cube, # target population from which to draw
                                  observed_U=F,
                                  complex_fit_models="ksvm",
...
# Save results
saveRDS(sim_results_knot_ksvm_t_o,
        "/Results/Simulation_results/sim_results_knot_ksvm_t_o.rds")
...
## ----run_sim_knot2_cs_t_o-------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_knot2_cs_t_o = data.frame(t(parSapply(cl,1:m,
               function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                  target_pop=sim_data_knot2_withknot, # target population from which to draw
                                  observed_U=F,
                                  complex_fit_models="correctly_specified",
                                  overlap_function="ifelse(X_target[,1]>qnorm(0.9),0,
                                  ifelse(X_target[,1]<qnorm(0.5),0,1))")}))) # whether U is observed #
...
# Save results
saveRDS(sim_results_knot2_cs_t_o,
        "/Results/Simulation_results/sim_results_knot2_cs_t_o.rds")
...
# # ## ----gen_data_knot3-------------------------------------------------------------------------------------------------------------------------------------------------
# # #####################################################################################################
# # Read in each object from sim_knot3
sim_data_knot3 = qread("/Results/Simulation_results/sim_data_knot3.qs")
clusterExport(cl=cl,"sim_data_knot3")

# Add knot term
sim_data_knot3_withknot = sim_data_knot3 %>% mutate(X1_knot = (X1 < 0.5)*(X1 - 0.5))
clusterExport(cl=cl,"sim_data_knot3_withknot")


# ## ----run_sim_knot3_cs_t_o-------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_knot3_cs_t_o = data.frame(t(parSapply(cl,1:m,
               function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                  target_pop=sim_data_knot3_withknot, # target population from which to draw
                                  observed_U=F,
                                  complex_fit_models="correctly_specified",
                                  overlap_function="ifelse(X_target[,1]>qnorm(0.9),0,
                                  ifelse(X_target[,1]<qnorm(0.5),0,1))")}))) # whether U is observed #
...
# Save results
saveRDS(sim_results_knot3_cs_t_o,
        "/Results/Simulation_results/sim_results_knot3_cs_t_o.rds")
...
# ## ----run_sim_UX1_cs_t_o---------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_UX1_cs_t_o = data.frame(t(parSapply(cl,1:m,
               function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                  target_pop=sim_data_UX1, # target population from which to draw
                                  observed_U=F,
                                  complex_fit_models="correctly_specified",
                                  overlap_function="ifelse(X_target[,1]>qnorm(0.9),0,
                                  ifelse(X_target[,1]<qnorm(0.5),0,1))")}))) # whether U is observed #
...
# Save results
saveRDS(sim_results_UX1_cs_t_o,
        "/Results/Simulation_results/sim_results_UX1_cs_t_o.rds")
...
# # ## ----run_sim_UX1_ksvm_t_o-------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_UX1_ksvm_t_o = parSapply(cl,1:m,
               function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                  target_pop=sim_data_UX1_noX1cube, # target population from which to draw
                                  observed_U=F,
                                  complex_fit_models="ksvm",
...
# Save results
saveRDS(sim_results_UX1_ksvm_t_o,
        "/Results/Simulation_results/sim_results_UX1_ksvm_t_o.rds")


## ----run_sim_U_measured_cs_t_o--------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_U_measured_cs_t_o = data.frame(t(parSapply(cl,1:m,
               function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                  target_pop=sim_data_complex, # target population from which to draw
                                  observed_U=T,
                                  complex_fit_models="correctly_specified",
...
# Save results
saveRDS(sim_results_U_measured_cs_t_o,
        "/Results/Simulation_results/sim_results_U_measured_cs_t_o.rds")
...
## ----run_sim_less_conf_cs_t_o---------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_less_conf_cs_t_o = data.frame(t(parSapply(cl,1:m,
               function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                  target_pop=sim_data_less_conf, # target population from which to draw
                                  observed_U=F,
                                  complex_fit_models="correctly_specified",
                                  overlap_function="ifelse(X_target[,1]>qnorm(0.9),0,
                                  ifelse(X_target[,1]<qnorm(0.5),0,1))")}))) # whether U is observed #
...
# Save results
saveRDS(sim_results_less_conf_cs_t_o,
        "/Results/Simulation_results/sim_results_less_conf_cs_t_o.rds")
...
## ----run_sim_more_conf_cs_t_o---------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_more_conf_t_o = data.frame(t(parSapply(cl,1:m,
               function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                  target_pop=sim_data_more_conf, # target population from which to draw
                                  observed_U=F,
                                  complex_fit_models="correctly_specified",
                                  overlap_function="ifelse(X_target[,1]>qnorm(0.9),0,
                                  ifelse(X_target[,1]<qnorm(0.5),0,1))")}))) # whether U is observed #
...
# Save results
saveRDS(sim_results_more_conf_t_o,
        "/Results/Simulation_results/sim_results_more_conf_cs_t_o.rds")
...
## ----run_sim_U_measured_ksvm_t_o------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_U_measured_ksvm_t_o = parSapply(cl,1:m,
               function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                  target_pop=sim_data_complex_noX1cube, # target population from which to draw
                                  observed_U=T,
                                  complex_fit_models="ksvm",
...
# Save results
saveRDS(sim_results_U_measured_ksvm_t_o,
        "/Results/Simulation_results/sim_results_U_measured_ksvm_t_o.rds")


## ----run_sim_less_conf_ksvm_to--------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_less_conf_ksvm_t_o = parSapply(cl,1:m,
               function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                  target_pop=sim_data_less_conf_noX1cube, # target population from which to draw
                                  observed_U=F,
                                  complex_fit_models="ksvm",
...
# Save results
saveRDS(sim_results_less_conf_ksvm_t_o,
        "/Results/Simulation_results/sim_results_to_less_conf.rds")

## ----run_sim_more_conf_ksvm_to--------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_more_conf_ksvm_t_o = parSapply(cl,1:m,
               function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                  target_pop=sim_data_more_conf_noX1cube, # target population from which to draw
                                  observed_U=F,
                                  complex_fit_models="ksvm",
...
# Save results
saveRDS(sim_results_more_conf_ksvm_t_o,
        "/Results/Simulation_results/sim_results_to_more_conf.rds")
...
# ----run_sim_less_overlap_ksvm_t_o-----------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_less_overlap_t_o = parSapply(cl,1:m,
                                         function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                             target_pop=sim_data_less_overlap_noX1cube, # target population from which to draw
                                                                             observed_U=F,
                                                                             complex_fit_models="ksvm",
                                                                             overlap_function="ifelse(X_target[,1]>qnorm(0.9),0,
                                  ifelse(X_target[,1]<qnorm(0.7),0,1))")}) # whether U is observed #


# Save results
saveRDS(sim_results_less_overlap_t_o,
        "/Results/Simulation_results/sim_results_to_less_overlap.rds")

# ----run_sim_less_overlap_cs_t_o-----------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_less_overlap_cs_t_o = parSapply(cl,1:m,
                                         function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                                      target_pop=sim_data_less_overlap_noX1cube, # target population from which to draw
                                                                                      observed_U=F,
                                                                                      complex_fit_models="correctly_specified",
...
# Save results
saveRDS(sim_results_less_overlap_cs_t_o,
        "/Results/Simulation_results/sim_results_less_overlap_cs_t_o.rds")
...
## ----run_sim_more_overlap_ksvm_t_o-----------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_more_overlap_t_o_ksvm = parSapply(cl,1:m,
                                                      function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                                     target_pop=sim_data_more_overlap_noX1cube, # target population from which to draw
                                                                                     observed_U=F,
                                                                                     complex_fit_models="ksvm",
                                                                                     overlap_function="ifelse(X_target[,1]>qnorm(0.9),0,
                                  ifelse(X_target[,1]<qnorm(0.1),0,1))")}) # whether U is observed #



# Save results
saveRDS(sim_results_more_overlap_t_o_ksvm,
        "/Results/Simulation_results/sim_results_to_more_overlap_ksvm.rds")

## ----run_sim_more_overlap_cs_t_o-----------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_more_overlap_cs_t_o = parSapply(cl,1:m,
                                              function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                                           target_pop=sim_data_more_overlap_noX1cube, # target population from which to draw
                                                                                           observed_U=F,
                                                                                           complex_fit_models="correctly_specified",
...
# Save results
saveRDS(sim_results_more_overlap_cs_t_o,
        "/Results/Simulation_results/sim_results_more_overlap_cs_t_o.rds")


# ----run_sim_less_overlap_ksvm-----------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_less_overlap_ksvm = parSapply(cl,1:m,
                                         function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                             target_pop=sim_data_less_overlap_noX1cube, # target population from which to draw
                                                                             observed_U=F,
                                                                             complex_fit_models="ksvm",
...
# Save results
saveRDS(sim_results_less_overlap_ksvm,
        "/Results/Simulation_results/sim_results_less_overlap_ksvm.rds")


## ----run_sim_more_overlap_ksvm-----------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_more_overlap_ksvm = parSapply(cl,1:m,
                                                      function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                                     target_pop=sim_data_more_overlap_noX1cube, # target population from which to draw
                                                                                     observed_U=F,
                                                                                     complex_fit_models="ksvm",
...
# Save results
saveRDS(sim_results_more_overlap_ksvm,
        "/Results/Simulation_results/sim_results_more_overlap_ksvm.rds")
...
## ----run_sim_cbv_cs_t_o---------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_cbv_cs_t_o = data.frame(t(parSapply(cl,1:m,
                                                function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                               target_pop=sim_data_cbv_withknot, # target population from which to draw
                                                                               observed_U=F,
                                                                               complex_fit_models="correctly_specified",
                                                                               overlap_function="ifelse(X_target[,1]>qnorm(0.9),0,
                                  ifelse(X_target[,1]<qnorm(0.5),0,1))")}))) # whether U is observed #
...
# Save results
saveRDS(sim_results_cbv_cs_t_o,
        "/Results/Simulation_results/sim_results_cbv_cs_t_o.rds")
...
## ----run_sim_cbv_ksvm_t_o-------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_cbv_ksvm_t_o = parSapply(cl,1:m,
                                                  function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                                 target_pop=sim_data_cbv_noX1cube, # target population from which to draw
                                                                                 observed_U=F,
                                                                                 complex_fit_models="ksvm",
...
# Save results
saveRDS(sim_results_cbv_ksvm_t_o,
        "/Results/Simulation_results/sim_results_cbv_ksvm_t_o.rds")
...
## ----run_sim_U_f_X1_cs_t_o------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_U_f_X1_cs_t_o = data.frame(t(parSapply(cl,1:m,
                                                   function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                                  target_pop=sim_data_U_f_X1, # target population from which to draw
                                                                                  observed_U=F,
                                                                                  complex_fit_models="correctly_specified",
                                                                                  overlap_function="ifelse(X_target[,1]>qnorm(0.9),0,
                                  ifelse(X_target[,1]<qnorm(0.5),0,1))")}))) # whether U is observed #
...
# Save results
saveRDS(sim_results_U_f_X1_cs_t_o,
        "/Results/Simulation_results/sim_results_U_f_X1_cs_t_o.rds")
...
## ----run_sim_U_f_X1_ksvm_t_o----------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_U_f_X1_ksvm_t_o = parSapply(cl,1:m,
                                                     function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                                    target_pop=sim_data_U_f_X1_noX1cube, # target population from which to draw
                                                                                    observed_U=F,
                                                                                    complex_fit_models="ksvm",
...
# Save results
saveRDS(sim_results_U_f_X1_ksvm_t_o,
        "/Results/Simulation_results/sim_results_U_f_X1_ksvm_t_o.rds")
...
## ----run_sim_SU_cs_t_o----------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_SU_cs_t_o = data.frame(t(parSapply(cl,1:m,
                                               function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                              target_pop=sim_data_SU, # target population from which to draw
                                                                              observed_U=F,
                                                                              complex_fit_models="correctly_specified",
                                                                              overlap_function="ifelse(X_target[,1]>qnorm(0.9),0,
                                  ifelse(X_target[,1]<qnorm(0.5),0,1))")}))) # whether U is observed #
...
# Save results
saveRDS(sim_results_SU_cs_t_o,
        "/Results/Simulation_results/sim_results_SU_cs_t_o.rds")
...
## ----run_sim_SU_ksvm_t_o--------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_SU_ksvm_t_o = parSapply(cl,1:m,
                                                 function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                                target_pop=sim_data_SU_noX1cube, # target population from which to draw
                                                                                observed_U=F,
                                                                                complex_fit_models="ksvm",
...
# Save results
saveRDS(sim_results_SU_ksvm_t_o,
        "/Results/Simulation_results/sim_results_SU_ksvm_t_o.rds")
...
## ----run_sim_1to1_ksvm_t_o------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_1to1_ksvm_t_o =parSapply(cl,1:m,
                                                   function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                                  target_pop=sim_data_1to1_noX1cube, # target population from which to draw
                                                                                  observed_U=F,
                                                                                  complex_fit_models="ksvm",
...
# Save results
saveRDS(sim_results_1to1_ksvm_t_o,
        "/Results/Simulation_results/sim_results_1to1_ksvm_t_o.rds")

## ----run_sim_1to1_cs_t_o------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_1to1_cs_t_o =parSapply(cl,1:m,
                                     function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                                  target_pop=sim_data_1to1_noX1cube, # target population from which to draw
                                                                                  observed_U=F,
                                                                                  complex_fit_models="correctly_specified",
                                                                                  overlap_function="ifelse(X_target[,1]>qnorm(0.98),0,
                                  ifelse(X_target[,1]<qnorm(0.18),0,1))")}) # whether U is observed #



# Save results
saveRDS(sim_results_1to1_cs_t_o,
        "/Results/Simulation_results/sim_results_1to1_cs_t_o.rds")
...
## ----run_sim_1to4_ksvm_t_o------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_1to4_ksvm_t_o = parSapply(cl,1:m,
                                                   function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                                  target_pop=sim_data_1to4_noX1cube, # target population from which to draw
                                                                                  observed_U=F,
                                                                                  complex_fit_models="ksvm",
...
# Save results
saveRDS(sim_results_1to4_ksvm_t_o,
        "/Results/Simulation_results/sim_results_1to4_ksvm_t_o.rds")

## ----run_sim_1to4_cs_t_o------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_1to4_cs_t_o = parSapply(cl,1:m,
                                      function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                                   target_pop=sim_data_1to4_noX1cube, # target population from which to draw
                                                                                   observed_U=F,
                                                                                   complex_fit_models="correctly_specified",
...
# Save results
saveRDS(sim_results_1to4_cs_t_o,
        "/Results/Simulation_results/sim_results_1to4_cs_t_o.rds")
...
## ----run_sim_1to30_ksvm_t_o-----------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_1to30_ksvm_t_o = parSapply(cl,1:m,
                                                    function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                                   target_pop=sim_data_1to30_noX1cube, # target population from which to draw
                                                                                   observed_U=F,
                                                                                   complex_fit_models="ksvm",
...
# Save results
saveRDS(sim_results_1to30_ksvm_t_o,
        "/Results/Simulation_results/sim_results_1to30_ksvm_t_o.rds")

## ----run_sim_1to30_cs_t_o-----------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_1to30_cs_t_o = parSapply(cl,1:m,
                                       function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                                    target_pop=sim_data_1to30_noX1cube, # target population from which to draw
                                                                                    observed_U=F,
                                                                                    complex_fit_models="correctly_specified",
...
# Save results
saveRDS(sim_results_1to30_cs_t_o,
        "/Results/Simulation_results/sim_results_1to30_cs_t_o.rds")
...
## ----run_sim_overlap1----------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_overlap1_ksvm = parSapply(cl,1:m,
function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                   target_pop = sim_data_complex_noX1cube, # target population from which to draw
                   observed_U = F, # whether U is observed
                   complex_fit_models = "ksvm", # whether to use ksvm or random forest (ranger) to fit models for Y, A, and S
...
# Save results
saveRDS(sim_results_overlap1_ksvm,
        "/Results/Simulation_results/sim_results_overlap1_ksvm.rds")

## ----run_sim_overlap1_cs----------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_overlap1_cs = parSapply(cl,1:m,
                                      function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                                   target_pop = sim_data_complex_noX1cube, # target population from which to draw
                                                                                   observed_U = F, # whether U is observed
                                                                                   complex_fit_models = "correctly_specified", # whether to use cs or random forest (ranger) to fit models for Y, A, and S
                                                                                   overlap_function = "overlap1")}) # # true overlap region or way to determine overlap (F=default, overlap1, overlap2, overlap3)



# Save results
saveRDS(sim_results_overlap1_cs,
        "/Results/Simulation_results/sim_results_overlap1_cs.rds")
...
# # ## ----run_sim_overlap2----------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_overlap2_ksvm = parSapply(cl,1:m,
function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                   target_pop = sim_data_complex_noX1cube, # target population from which to draw
                   observed_U = F, # whether U is observed
                   complex_fit_models = "ksvm", # whether to use ksvm or random forest (ranger) to fit models for Y, A, and S
...
# Save results
saveRDS(sim_results_overlap2_ksvm,
        "/Results/Simulation_results/sim_results_overlap2_ksvm.rds")

## ----run_sim_overlap2_cs----------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_overlap2_cs = parSapply(cl,1:m,
                                    function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                                 target_pop = sim_data_complex_noX1cube, # target population from which to draw
                                                                                 observed_U = F, # whether U is observed
                                                                                 complex_fit_models = "correctly_specified", # whether to use cs or random forest (ranger) to fit models for Y, A, and S
...
# Save results
saveRDS(sim_results_overlap2_cs,
        "/Results/Simulation_results/sim_results_overlap2_cs.rds")
...
# # ## ----run_sim_overlap3----------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_overlap3_ksvm = parSapply(cl,1:m,
function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                   target_pop = sim_data_complex_noX1cube, # target population from which to draw
                   observed_U = F, # whether U is observed
                   complex_fit_models = "ksvm", # whether to use ksvm or random forest (ranger) to fit models for Y, A, and S
...
# Save results
saveRDS(sim_results_overlap3_ksvm,
        "/Results/Simulation_results/sim_results_overlap3_ksvm.rds")

## ----run_sim_overlap3_cs----------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_overlap3_cs = parSapply(cl,1:m,
                                    function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                                 target_pop = sim_data_complex_noX1cube, # target population from which to draw
                                                                                 observed_U = F, # whether U is observed
                                                                                 complex_fit_models = "correctly_specified", # whether to use cs or random forest (ranger) to fit models for Y, A, and S
                                                                                 overlap_function = "overlap3")}) # # true overlap region or way to determine overlap (F=default, overlap3, overlap2, overlap3)



# Save results
saveRDS(sim_results_overlap3_cs,
        "/Results/Simulation_results/sim_results_overlap3_cs.rds")
...
# # ## ----run_sim_overlap4----------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_overlap4_ksvm = parSapply(cl,1:m,
function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                   target_pop = sim_data_complex_noX1cube, # target population from which to draw
                   observed_U = F, # whether U is observed
                   complex_fit_models = "ksvm", # whether to use ksvm or random forest (ranger) to fit models for Y, A, and S
...
# Save results
saveRDS(sim_results_overlap4_ksvm,
        "/Results/Simulation_results/sim_results_overlap4_ksvm.rds")

## ----run_sim_overlap4_cs----------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_overlap4_cs = parSapply(cl,1:m,
                                    function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                                 target_pop = sim_data_complex_noX1cube, # target population from which to draw
                                                                                 observed_U = F, # whether U is observed
                                                                                 complex_fit_models = "correctly_specified", # whether to use cs or random forest (ranger) to fit models for Y, A, and S
                                                                                 overlap_function = "overlap4")}) # # true overlap region or way to determine overlap (F=default, overlap4, overlap2, overlap3)



# Save results
saveRDS(sim_results_overlap4_cs,
        "/Results/Simulation_results/sim_results_overlap4_cs.rds")


# # # ## ----run_sim_rf_t_o-------------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_rf_t_o = parSapply(cl,1:m,
                                            function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                           target_pop=sim_data_complex_noX1cube, # target population from which to draw
                                                                           observed_U=F,
                                                                           complex_fit_models="ranger", # whether to use ksvm or random forest (ranger) to fit models for Y, A, and S
                                                                           overlap_function="ifelse(X_target[,1]>qnorm(0.9),0,
                                  ifelse(X_target[,1]<qnorm(0.5),0,1))")}) # whether U is observed #
...
# Save results
saveRDS(sim_results_rf_t_o,
        "/Results/Simulation_results/sim_results_rf_t_o2.rds")


## ----run_sim_cbv_ranger_t_o----------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_cbv_ranger_t_o = parSapply(cl,1:m,
                                                    function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                                   target_pop=sim_data_cbv_noX1cube, # target population from which to draw
                                                                                   observed_U=F,
                                                                                   complex_fit_models="ranger",
                                                                                   overlap_function="ifelse(X_target[,1]>qnorm(0.9),0,
                                  ifelse(X_target[,1]<qnorm(0.5),0,1))")}) # whether U is observed #
...
# Save results
saveRDS(sim_results_cbv_ranger_t_o,
        "/Results/Simulation_results/sim_results_cbv_ranger_t_o.rds")

## ----run_sim_U_f_X1_ranger_t_o----------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_U_f_X1_ranger_t_o = parSapply(cl,1:m,
                                                       function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                                      target_pop = sim_data_U_f_X1_noX1cube, # target population from which to draw
                                                                                      observed_U = F, # whether U is observed
                                                                                      complex_fit_models = "ranger",
...
# Save results
saveRDS(sim_results_U_f_X1_ranger_t_o,
        "/Results/Simulation_results/sim_results_U_f_X1_ranger_t_o.rds")
...
## ----run_sim_s_probabilisticcs_t_o-----------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_s_probabilistic_cs_t_o = parSapply(cl,1:m,
                                              function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                                           target_pop=sim_data_s_probabilistic, # target population from which to draw
                                                                                           observed_U=F,
                                                                                           complex_fit_models="correctly_specified",
                                                                                           overlap_function="ifelse(X_target[,1]>qnorm(0.9),0,
                                  ifelse(X_target[,1]<qnorm(0.5),0,1))")}) 



# Save results
saveRDS(sim_results_s_probabilistic_cs_t_o,
        paste0("/Results/Simulation_results/sim_results_s_probabilistic_cs_t_o.rds"))
...
#####################################################################################################################
#####################################################################################################################
# # ## ----gen_data_s_probabilistic_b-------------------------------------------------------------------------------------------------------------------------------------------------
# # #####################################################################################################
# Read in each object from sim_s_probabilistic_b
sim_data_s_probabilistic_b = qread("/Results/Simulation_results/sim_data_s_probabilistic_b.qs")
clusterExport(cl=cl,"sim_data_s_probabilistic_b")

sim_data_s_probabilistic_b_noX1cube = subset(sim_data_s_probabilistic_b,select=c(-X1_cube))
clusterExport(cl=cl,"sim_data_s_probabilistic_b_noX1cube")

## ----run_sim_s_probabilistic_b_cs-----------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_s_probabilistic_b_cs = parSapply(cl,1:m,
                                               function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                                            target_pop=sim_data_s_probabilistic_b, # target population from which to draw
                                                                                            observed_U=F,
                                                                                            complex_fit_models=F,
                                                                                            overlap_function=F)}) 



# Save results
saveRDS(sim_results_s_probabilistic_b_cs,
        paste0("/Results/Simulation_results/sim_results_s_probabilistic_b_cs.rds"))


## ----run_sim_s_probabilistic_b_ensemble-----------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_s_probabilistic_b_ensemble = parSapply(cl,1:m,
                                                 function(i,...){run_simulation_remove_errors(n_target = my_n_target, # target sample size
                                                                                              target_pop=sim_data_s_probabilistic_b_noX1cube, # target population from which to draw
                                                                                              observed_U=F,
                                                                                              complex_fit_models="ensemble",
                                                                                              overlap_function=F)}) # whether U is observed #
...
# Save results
saveRDS(sim_results_s_probabilistic_b_ensemble,
        paste0("/Results/Simulation_results/sim_results_s_probabilistic_b_ensemble",file_suffix,".rds"))
...
## ----run_sim_boot_cs_t_o-----------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_boot_cs_t_o = parLapply(cl,1:m,
                                    function(i,...){run_simulation_with_bootstrap(n_target = my_n_target, # target sample size #
                                                                                  target_pop=sim_data_complex, # target population from which to draw
                                                                                  observed_U=F,
                                                                                  complex_fit_models="correctly_specified", # whether to use ensemble or random forest (ranger) to fit models for Y, A, and S
                                                                                  SL_library=my_SL_library, # specifies SuperLearner library for complex_fit_models="ensemble"
                                                                                  overlap_function="ifelse(X_target[,1]>qnorm(0.9),0,
                                                               ifelse(X_target[,1]<qnorm(0.5),0,1))",
                                                                                  M=1000)}) 
...
# Save results
saveRDS(sim_results_boot_cs_t_o,
        paste0("/Results/Simulation_results/sim_results_boot_cs_t_o.rds"))


## ----run_sim_boot_t_o_linear-----------------------------------------------------------------------------------------------------------------------------------------------
clusterSetRNGStream(cl, iseed = random_seed)
sim_results_boot_t_o_linear = parLapply(cl,1:m,
                                    function(i,...){run_simulation_with_bootstrap(n_target = my_n_target, # target sample size #
                                                                                  target_pop=sim_data_complex_noX1cube, # target population from which to draw
                                                                                  observed_U=F,
                                                                                  complex_fit_models=F, # whether to use ensemble or random forest (ranger) to fit models for Y, A, and S
                                                                                  SL_library=my_SL_library, # specifies SuperLearner library for complex_fit_models="ensemble"
                                                                                  overlap_function="ifelse(X_target[,1]>qnorm(0.9),0,
                                                               ifelse(X_target[,1]<qnorm(0.5),0,1))",
                                                                                  M=1000)}) 
...
# Save results
saveRDS(sim_results_boot_t_o_linear,
        paste0("/Results/Simulation_results/sim_results_boot_t_o_linear.rds"))
