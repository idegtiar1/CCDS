...
# Base case results
Let's discuss the estimators one at a time to determine when they work well and when they don't. Why can't we just use the rand estimator? With correctly specified models, the rand estimator is able to extrapolate well; it would be a natural choice in this setting. However, we never know that we've fit the correct models in practice, and when we (incorrectly) fit simple linear models, even though the true cubic relationship isn't too far off the linear one, the rand estimator is no longer able to extrapolate well beyond its support, resulting in large external validity bias.

The next easiest approach is to use the obs/rand estimator. However, this estimator is subject to unmeasured confounding bias, which exists even when correctly-specified models are fit.

Let's jump ahead and focus on the 2-stage WD estimator, the last in the list. With correctly specified models, all novel estimators including the 2-stage WD approach are able to decrease unmeasured confounding bias. And the 2-stage WD approach is most efficient novel outcome regression estimator. However, when (incorrectly) fitting simple linear models, just as with the rand estimator, the issue of extrapolation similarly plagues the 2-stage WD estimator. In fact, with simple linear least-squares models, its estimates will be identical to that of the rand estimator and with more complex ensemble models its bias tends to be similar to that of the rand estimator. Because of the 2-stage WD's extreme sensitivity to model misspecificiation (which we also observe when fitting ensemble regressions), I do not generally recommend using the 2-stage WD estimator. 
...
Across all these settings, we note that in particular when fitting the flexible ensemble approaches, bias and RMSE may not decrease necessarily compared to the obs/rand estimator. This is due to remnant modeling bias. This bias does shrink with more overlap as well as with more randomized data (see subsequent sections of this report).  

*PAPER FIGURE 2* (figure:results_base_case)  
...
plot_bias_rmse_results(my_tables)  
```
...
# Coverage
## Main results
All novel estimators were able to achieve nominal coverage except the CCDS-IPW estimator when using grossly misspecified linear models and except the 2-stage WD estimator in the linear model and ensemble setting. The obs/rand estimator attained 0% coverage across all settings, and the rand estimator attained 0% coverage in the linear settings for the PATMs but 95% coverage for the PATE due to biases cancelling out.

*PAPER FIGURE 3* (figure:results_coverge)  
...
plot_coverage_results(my_tables)     
...
plot_coverage_results(my_tables_lu2019)     
...
With correctly specified models, although all estimators' bias is lower than that of the obs/rand estimator, the ensemble CCDS and CCDS-AIPW estimators retains a non-trivial amount of bias that minimally decreases with larger sample sizes due to modeling bias from fitting models in the small overlap region. With smaller sample sizes ($n=2000$; $n_\text{RCT}=500$), the RMSE of all novel estimators and the rand estimator exceed that of the obs/rand estimator. 

With ensemble models, all novel estimators' bias is below that of the obs/rand estimator (besides the 2-stage WD, which drops below after $n=10,000$). RMSE, however, only drops below that of the obs/rand estimator with $n=50,000$ (except for the CCDS-IPW estimator, whose RMSE is lower even with $n=2,000$.
...
plot_bias_rmse_separately(my_tables_b, my_estimands = c("E(Y^1) - E(Y^2)"),
                          estimators_remove = c("Oracle", "2-stage CCDS", "2-stage hybrid CCDS", 
                                                         ...
                                                         "Lu-rand", "Lu-obs/rand1", "Lu-obs/rand2")) + 
...
With more overlap, all estimators are able to shrink bias close to zero. With less overlap, the rand estimator and CCDS/CCDS-AIPW estimators particularly suffer.
...
Bias and RMSE decreases across all estimators as the ratio of RCT to obs observations increases. As the RCT observations comprise a targer portion of the target sample, the bias from using the rand and obs/rand estimators also necessarily decreases. 
...
A violation of the constant bias assumption increases bias and RMSE for each of the estimators corresponding to the amount of extra unmeasured confounding bias observed in the obs/rand estimator. The impact purely accommodates the extra unmeasured confounding bias which was not removed.
...
my_tables = rbind(cbind("Type" = table_names[2], "Setting"="default", table_ensemble),
  ...
  mutate(Setting = recode(Setting, "overlap1" =  0.25, 
                                            ...
                                            "overlap3"= 0.91)) %>% 
  mutate(Type = factor(Type, table_names)) %>% 
  filter(Estimator %notin% c("Rand","Obs/rand"))
...
my_tables[my_tables$Estimator %notin% c("Oracle", "2-stage CCDS", "2-stage hybrid CCDS", "weighted 2-stage hybrid CCDS", 
    ...
    mutate(Estimator = factor(Estimator,levels = c("Rand","Obs/rand"," 2-stage WD",
